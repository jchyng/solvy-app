...
- [ ] `PATCH /api/v1/conversations/:id` 단위 테스트 — `is_favorite`, `title` 필드 갱신 검증 (DB mock)
- [ ] 자동 제목 생성 테스트 — `ai.nameNote` mock으로 `auto_title` 저장 확인
- [ ] 목록 우선순위 테스트 — `title` > `auto_title` > "제목 없음" 분기 검증
- [ ] 소프트 삭제 테스트 — DELETE 시 `deleted_at` 설정, 목록 조회에서 제외 확인
- [ ] 폴더 CRUD 단위 테스트 — 생성·수정·삭제·순서 변경 (DB mock)
- [ ] 대화-폴더 다대다 관계 테스트 — 하나의 대화가 여러 폴더에 추가·제거
- [ ] **소유권 체크 테스트** — 다른 사용자 JWT로 폴더·대화 수정 시 403 반환 확인
- [ ] 노트 홈 컴포넌트 테스트 — 탭(최근/즐겨찾기/목록) 전환, 카드 렌더링
- [ ] 폴더 바텀시트 컴포넌트 테스트 — 생성·멀티 선택·체크박스 상태
- [ ] `npm test` 에러 없음
